...
## Load spreadsheets
AHarvestRaw <- as.data.table(read_excel("Raw data/Harvest_and_public/Harvest starting data/2023 Statewide Pronghorn Harvest_Raw.xlsx", sheet="Normal"))
#AHarvestSC <- as.data.table(read_excel("Raw data/Harvest_and_public/2023 Statewide Elk Harvest_Raw.xlsx", sheet="SC"))
...
AHarvestFinal23<-AHarvestRaw[Unit!="Total"]

#combine doe and fawn into "Antlerless"
AHarvestFinal23$Does<-as.numeric(AHarvestFinal23$Does)
AHarvestFinal23$Fawns<-as.numeric(AHarvestFinal23$Fawns)

AHarvestFinal23[,"Antlerless":=Does + Fawns]


write_xlsx(AHarvestFinal23,'Raw data/Harvest_and_public/AntHarvest23Final.xlsx')
