...
## Load spreadsheets
AHarvestRaw <- as.data.table(read_excel("Raw data/Harvest_and_public/Harvest starting data/2022 Statewide Pronghorn Harvest_Raw.xlsx", sheet="Normal"))
#AHarvestSC <- as.data.table(read_excel("Raw data/Harvest_and_public/2022 Statewide Elk Harvest_Raw.xlsx", sheet="SC"))
...
AHarvestFinal22<-AHarvestRaw[Unit!="Total"]

#combine doe and fawn into "Antlerless"
AHarvestFinal22$Does<-as.numeric(AHarvestFinal22$Does)
AHarvestFinal22$Fawns<-as.numeric(AHarvestFinal22$Fawns)

AHarvestFinal22[,"Antlerless":=Does + Fawns]


write_xlsx(AHarvestFinal22,'Raw data/Harvest_and_public/AntHarvest22Final.xlsx')
