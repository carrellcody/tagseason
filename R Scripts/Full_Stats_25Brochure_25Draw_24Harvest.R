...
highdemand<-as.data.table(read_excel("Raw data/CPW/2025 BG Hunt codes for Brochures.xlsx", 
                                     sheet = "High Demand"))
hybrid<-as.data.table(read_excel("Raw data/CPW/2025 BG Hunt codes for Brochures.xlsx", 
                                     sheet = "Hybrid"))
...
GMUDeer25<-as.data.table(read_excel("Raw data/CPW/2025 BG Hunt codes for Brochures.xlsx", 
                                    sheet = "Deer"))
...
GMUDeer25[, WTO := ifelse(grepl("WTO", tblsort), "White-tailed Deer Only", "")]


#remove unnecessary columns
GMUDeer25[,c('ID','spp','tblsort','row_cnt','MandatoryCWD','nr_lic_available','UNIT','...17'):=NULL]

#remove comments from the list of units

# Extract numbers
GMUDeer25[, All_Units := trimws(regmatches(AUNIT, regexpr("(\\d+\\s*,\\s*)*\\d+", AUNIT)))]

# Extract text (everything NOT part of the number list)
GMUDeer25[, Text := trimws(gsub("(\\d+\\s*,\\s*)*\\d+", "", AUNIT))]

#remove all repeats now that the individual columns are gone
GMUDeer25<-unique(GMUDeer25)


#collapse dates from repeated units into one list of dates
GMUDeer25<-GMUDeer25[, .(DATES = paste(DATES, collapse = ",")), 
                     by = setdiff(names(GMUDeer25), "DATES")]
...
FullDeer25 <- longDOADeer25[
  GMUDeer25[, .(HUNT_CODE, All_Units, DATES, All_Notes)],  # only these columns
  on = .(Tag = HUNT_CODE),
  `:=`(
    `Valid GMUs` = i.All_Units,
    Dates        = i.DATES,
    Notes        = i.All_Notes
  )
]
...
FullDeer25[Tag%in%hybrid$'2025',hybrid:="Hybrid"]
...
setnames(FullDeer25_Longunits,"Valid GMUs","Unit")
...
FullDeer25_LongunitsMerged <- copy(FullDeer25_Longunits)
...
tags_by_unit <- FullDeer25_Longunits[
...
###Split out OTC table and add OTC designation Add OTC designation for harvest stats----
#OTC Designation
WTOnlyLateRifleOTC<-c(48, 49, 56, 57, 58, 59, 69, 84, 85, 86,
                      140, 481, 511, 512, 561, 581, 591, 691, 851, 861)
...
DeerOTCHarvest24[!is.na(OTCCat)]


#write CSVs----

write_csv(FullDeer25Final, file='Website/FullDeer25Final.csv')
...
GMUelk25<-as.data.table(read_excel("Raw data/CPW/2025 BG Hunt codes for Brochures.xlsx", 
                                   sheet = "ELK"))
...
#remove unnecessary columns
GMUelk25[,c('ID','spp','tblsort','row_cnt','MandatoryCWD','nr_lic_available','UNIT','...17'):=NULL]

#remove comments from the list of units

# Extract numbers
GMUelk25[, All_Units := trimws(regmatches(AUNIT, regexpr("(\\d+\\s*,\\s*)*\\d+", AUNIT)))]

# Extract text (everything NOT part of the number list)
GMUelk25[, Text := trimws(gsub("(\\d+\\s*,\\s*)*\\d+", "", AUNIT))]

#remove all repeats now that the individual columns are gone
GMUelk25<-unique(GMUelk25)


#collapse dates from repeated units into one list of dates
GMUelk25<-GMUelk25[, .(DATES = paste(DATES, collapse = ",")), 
                   by = setdiff(names(GMUelk25), "DATES")]
#combine the note columns and ignore NAs
collapse_nonempty <- function(...) {
  x <- unlist(list(...))
  x <- x[!is.na(x) & trimws(x) != ""]
...
Fullelk25 <- longDOAelk25[
  GMUelk25[, .(HUNT_CODE, All_Units, DATES, All_Notes)],  # only these columns
  on = .(Tag = HUNT_CODE),
  `:=`(
    `Valid GMUs` = i.All_Units,
    Dates        = i.DATES,
...
Fullelk25[Tag%in%hybrid$'2025',hybrid:="Hybrid"]
...
setnames(Fullelk25_Longunits,"Valid GMUs","Unit")
...
ArcheryAntlerlessOTC<-c(6, 16, 17, 18, 21, 22, 30, 31,
                        32, 38, 43, 82, 85, 86, 133, 134, 140,
                        141, 142, 161, 171, 181,
                        431, 471, 682, 691, 791, 851)
...
ArcheryEitherSexOTC<-c(3, 6, 11, 13, 14, 15, 16, 17, 18, 21, 22,
                       ...
                       691, 851,
                       861, 951)
ArcheryEitherSexOTCEastNonRes<-c(87, 88, 89, 90, 91, 92, 93, 94, 95,
                                 ...
                                 143, 144, 145, 146, 147, 951)

RifleOTC<-c(6, 15, 16, 17,
            18, 21, 22, 25, 26, 27, 28, 30,
            31, 32, 33, 34, 35, 36, 37, 38,
            41, 42, 43, 44, 45, 47, 52,
            53, 54, 55, 59, 60, 62, 63, 64,
            65, 68, 70, 71, 72, 73, 74, 75,
            77, 78, 80, 81, 82, 85, 86, 133, 134,
            140, 141, 142, 161, 171, 181,
            361, 371, 411, 421, 431, 444,
            471, 511, 521, 551, 581, 591,
            681, 691, 711, 741, 751, 771,
            851, 861)
RifleOTCPLO<-c(3, 4,
               5, 11, 12, 13, 14, 23, 24, 131,
               211, 214, 231, 301, 441)
...
elkOTCHarvest24[Category=="Third Rifle Seasons (All Harvest - Includes PLOs)" & Unit%in%RifleOTCPLO,
             OTC3RPLO:="Third Season Antlered - Private Land Only"]
elkOTCHarvest24[Category=="Second Rifle Seasons (All Harvest - Includes PLOs)" & Unit%in%RifleOTCPLO,
             OTC2RPLO:="Second Season Antlered - Private Land Only"]
...
elkOTCHarvest24<-data.table::melt(elkOTCHarvest24,
     measure.vars=c('OTCAA','OTCAE','OTC2R','OTC3R','OTCplainsR','OTC3RPLO','OTC2RPLO','OTCAplains','OTCAplainsF'),
     variable.name = "OTC",
     value.name = "OTCCat"
     )
...
#write CSV Elk----
write_csv(Fullelk25Final, file='Website/Fullelk25Final.csv')
...
GMUant25<-as.data.table(read_excel("Raw data/CPW/2025 BG Hunt codes for Brochures.xlsx", 
                                   sheet = "Pronghorn"))
...
#remove unnecessary columns
GMUant25<-GMUant25[spp=="Pronghorn"]
GMUant25[,c('ID','spp','tblsort','row_cnt','MandatoryCWD','nr_lic_available','UNIT','...17','...22'):=NULL]

#remove comments from the list of units

# Extract numbers
GMUant25[, All_Units := trimws(regmatches(AUNIT, regexpr("(\\d+\\s*,\\s*)*\\d+", AUNIT)))]

# Extract text (everything NOT part of the number list)
GMUant25[, Text := trimws(gsub("(\\d+\\s*,\\s*)*\\d+", "", AUNIT))]

#remove all repeats now that the individual columns are gone
GMUant25<-unique(GMUant25)


#collapse dates from repeated units into one list of dates
GMUant25<-GMUant25[, .(DATES = paste(DATES, collapse = ",")), 
                   by = setdiff(names(GMUant25), "DATES")]
#combine the note columns and ignore NAs
collapse_nonempty <- function(...) {
  x <- unlist(list(...))
  x <- x[!is.na(x) & trimws(x) != ""]
...
Fullant25 <- longDOAant25[
  GMUant25[, .(HUNT_CODE, All_Units, DATES, All_Notes)],  # only these columns
  on = .(Tag = HUNT_CODE),
  `:=`(
    `Valid GMUs` = i.All_Units,
    Dates        = i.DATES,
...
Fullant25[Tag%in%hybrid$'2025',hybrid:="Hybrid"]
...
setnames(Fullant25_Longunits,"Valid GMUs","Unit")
...
ArcheryEitherSexAntOTC<-c(1, 7, 8, 9, 14, 15, 19, 20, 22,
...
#writeCSVs Antelope----
write_csv(Fullant25Final, file='Website/Fullant25Final.csv')
write_csv(antHarvest25,'Website/antHarvest25.csv')
write_csv(antOTCHarvest24,'Website/antOTC24.csv')
